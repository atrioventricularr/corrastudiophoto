...
echo "========================================"
echo " Phase 9A3F - Countdown Capture Foundation"
echo "========================================"

mkdir -p apps/booth-ui/src/camera

cat > apps/booth-ui/src/camera/CameraCountdownPanel.tsx <<'TSX'
import React, {
  useEffect,
  useState,
} from 'react';
...
grep -q "CameraCountdownPanel" apps/booth-ui/src/camera/index.ts || cat >> apps/booth-ui/src/camera/index.ts <<'TS'
...
if "CameraCountdownPanel" not in text:
    ...
    lines.insert(
        insert_at,
        "import { CameraCountdownPanel } from './CameraCountdownPanel';",
    )
    text = "\n".join(lines) + "\n"

if "<CameraCountdownPanel />" not in text:
    marker = "<CameraCaptureGuidePanel />"

    if marker not in text:
        raise SystemExit(
            "Could not find <CameraCaptureGuidePanel />. Put <CameraCountdownPanel /> below it manually."
        )
...
echo ""
echo "Relevant lines:"
grep -R "CameraCountdownPanel\\|Start Countdown\\|captured-placeholder" -n apps/booth-ui/src/camera || true
