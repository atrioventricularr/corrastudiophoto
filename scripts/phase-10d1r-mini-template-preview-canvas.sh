...
mkdir -p apps/booth-ui/src/components/templates
...
cat > apps/booth-ui/src/components/templates/index.ts <<'TS'
export * from './TemplatePreviewCanvas';
TS
...
if "TemplatePreviewCanvas" not in text:
    text = text.replace(
        "import { useLayouts } from '../../layouts';",
        "import { useLayouts } from '../../layouts';\nimport { TemplatePreviewCanvas } from '../templates';",
    )

marker = """      <div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">
          Template Details
        </p>"""
...
if "<TemplatePreviewCanvas template={activeTemplate}" not in text:
    if marker not in text:
        raise SystemExit("Could not find Template Details marker.")
