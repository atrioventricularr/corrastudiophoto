...
MAIN_REQUIRE="require('./corra-disk-persistence.cjs');"
PRELOAD_REQUIRE="require('./corra-disk-preload.cjs');"
...
# Keep shebang if any, otherwise add at top.
if text.startswith("#!"):
    parts = text.split("\n", 1)
...
grep -R \
  ...
  --exclude-dir=dist \
  --exclude-dir=.vite \
  "corra-disk-persistence\|corra-disk-preload" \
