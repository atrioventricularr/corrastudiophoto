...
echo "========================================"
echo " Phase 8E3C1 - Session Lifecycle Sync Helper"
echo "========================================"

mkdir -p apps/booth-ui/src/sessions

cat > apps/booth-ui/src/sessions/supabase-session-lifecycle-sync.ts <<'TS'
import type {
  CorraBoothSession,
...
grep -q "supabase-session-lifecycle-sync" apps/booth-ui/src/sessions/index.ts || cat >> apps/booth-ui/src/sessions/index.ts <<'TS'
export * from './supabase-session-lifecycle-sync';
TS

if [ ! -f "apps/booth-ui/.env.local" ]; then
  cat > apps/booth-ui/.env.local <<'ENV'
VITE_RECORD_BOOTH_SESSION_LIFECYCLE_URL=https://uitnzrstkjvwiojbnpwg.supabase.co/functions/v1/record-booth-session-lifecycle
VITE_SUPABASE_ANON_KEY=
ENV
else
  grep -q "VITE_RECORD_BOOTH_SESSION_LIFECYCLE_URL" apps/booth-ui/.env.local || cat >> apps/booth-ui/.env.local <<'ENV'
...
echo ""
echo "Created:"
echo "- apps/booth-ui/src/sessions/supabase-session-lifecycle-sync.ts"
echo ""
echo "Patched:"
echo "- apps/booth-ui/src/sessions/index.ts"
echo "- apps/booth-ui/.env.local"
echo ""
echo "Phase 8E3C1 completed."
