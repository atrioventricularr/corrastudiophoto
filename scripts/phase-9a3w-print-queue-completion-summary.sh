...
echo "========================================"
echo " Phase 9A3W - Print Queue Completion Summary"
echo "========================================"

mkdir -p apps/booth-ui/src/camera

cat > apps/booth-ui/src/camera/CameraPrintQueueSummaryPanel.tsx <<'TSX'
...
grep -q "CameraPrintQueueSummaryPanel" apps/booth-ui/src/camera/index.ts || cat >> apps/booth-ui/src/camera/index.ts <<'TS'
...
if "CameraPrintQueueSummaryPanel" not in text:
    ...
    lines.insert(
        insert_at,
        "import { CameraPrintQueueSummaryPanel } from './CameraPrintQueueSummaryPanel';",
    )
    text = "\n".join(lines) + "\n"

if "<CameraPrintQueueSummaryPanel />" not in text:
    marker = "<CameraPrintQueuePanel />"

    if marker not in text:
        raise SystemExit(
            "Could not find <CameraPrintQueuePanel />. Put <CameraPrintQueueSummaryPanel /> below it manually."
        )

    text = text.replace(
        marker,
        marker + "\n      <CameraPrintQueueSummaryPanel />",
        1,
    )
...
echo ""
echo "Relevant lines:"
grep -R "CameraPrintQueueSummaryPanel\\|Print Queue Summary\\|Latest Job" -n apps/booth-ui/src/camera || true
