...
echo "========================================"
echo " Phase 9B8 - Booth Delivery Print Download"
echo "========================================"

mkdir -p apps/booth-ui/src/booth

cat > apps/booth-ui/src/booth/BoothDeliveryStep.tsx <<'TSX'
...
grep -q "BoothDeliveryStep" apps/booth-ui/src/booth/index.ts || cat >> apps/booth-ui/src/booth/index.ts <<'TS'
...
if "BoothDeliveryStep" not in text:
    ...
    lines.insert(insert_at, "import { BoothDeliveryStep } from './BoothDeliveryStep';")
    text = "\n".join(lines) + "\n"

if "<BoothDeliveryStep />" not in text:
    pattern = re.compile(
        r"""\{currentStep === 'delivery' && \(
            <div className="mt-4">
    ...
    text2 = pattern.sub("{currentStep === 'delivery' && <BoothDeliveryStep />}", text, count=1)

    if text2 == text:
        raise SystemExit("Could not replace delivery block. Inspect BoothCustomerScreen.tsx manually.")
...
echo ""
echo "Relevant lines:"
grep -R "BoothDeliveryStep\\|Print or Download\\|Download Final PNG\\|Print Now" -n apps/booth-ui/src/booth || true
