...
echo "========================================"
echo " Phase 8E2 - Admin Session Lifecycle Panel"
echo "========================================"

mkdir -p apps/booth-ui/src/components/admin

cat > apps/booth-ui/src/components/admin/SessionLifecyclePanel.tsx <<'TSX'
...
[ -f "$FILE" ] || {
  ...
  echo "SessionLifecyclePanel component was created, but AdminPanel patch skipped."
  exit 1
}
...
if "SessionLifecyclePanel" not in text:
    ...
    lines.insert(insert_at, "import { SessionLifecyclePanel } from './admin/SessionLifecyclePanel';")
    text = "\n".join(lines) + "\n"

if "<SessionLifecyclePanel" not in text:
    # Prefer placing after PaymentTransactionsPanel if it exists.
    if "<PaymentTransactionsPanel" in text:
        text = re.sub(
    ...
    elif "<PaymentSettingsPanel" in text:
        text = re.sub(
            r"(\s*<PaymentSettingsPanel[^>]*/>\s*)",
            r"\1\n        <SessionLifecyclePanel />\n",
            text,
            count=1,
        )
    else:
        # Fallback: insert before the final closing main div if possible.
        last_div = text.rfind("</div>")
        if last_div == -1:
            raise SystemExit("Could not find a safe place to insert SessionLifecyclePanel.")
...
grep -q "SessionLifecyclePanel" apps/booth-ui/src/components/admin/SessionLifecyclePanel.tsx || {
  echo "ERROR: SessionLifecyclePanel component missing."
  exit 1
}

grep -q "SessionLifecyclePanel" apps/booth-ui/src/components/AdminPanel.tsx || {
  echo "ERROR: AdminPanel missing SessionLifecyclePanel."
  exit 1
}

echo ""
echo "AdminPanel references:"
grep -n "SessionLifecyclePanel\\|PaymentTransactionsPanel\\|PaymentSettingsPanel" apps/booth-ui/src/components/AdminPanel.tsx || true

echo ""
echo "Phase 8E2 completed."
