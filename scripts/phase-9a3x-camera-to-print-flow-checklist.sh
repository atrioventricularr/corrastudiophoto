...
echo "========================================"
echo " Phase 9A3X - Camera to Print Flow Checklist"
echo "========================================"

mkdir -p apps/booth-ui/src/camera

cat > apps/booth-ui/src/camera/CameraToPrintFlowChecklistPanel.tsx <<'TSX'
import React, {
  useEffect,
  useMemo,
  useState,
...
grep -q "CameraToPrintFlowChecklistPanel" apps/booth-ui/src/camera/index.ts || cat >> apps/booth-ui/src/camera/index.ts <<'TS'
...
if "CameraToPrintFlowChecklistPanel" not in text:
    ...
    lines.insert(
        insert_at,
        "import { CameraToPrintFlowChecklistPanel } from './CameraToPrintFlowChecklistPanel';",
    )
    text = "\n".join(lines) + "\n"

if "<CameraToPrintFlowChecklistPanel />" not in text:
    marker = "<CameraPrintQueueSummaryPanel />"

    if marker in text:
        text = text.replace(
            marker,
            marker + "\n      <CameraToPrintFlowChecklistPanel />",
            1,
        )
    else:
        fallback = "<CameraPrintQueuePanel />"

        if fallback not in text:
            raise SystemExit(
                "Could not find CameraPrintQueueSummaryPanel or CameraPrintQueuePanel. Put <CameraToPrintFlowChecklistPanel /> below print queue manually."
            )
...
echo ""
echo "Relevant lines:"
grep -R "CameraToPrintFlowChecklistPanel\\|Camera to Print Flow\\|End-to-End Flow Complete" -n apps/booth-ui/src/camera || true
