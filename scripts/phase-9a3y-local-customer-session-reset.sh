...
echo "========================================"
echo " Phase 9A3Y - Local Customer Session Reset"
echo "========================================"

mkdir -p apps/booth-ui/src/camera

cat > apps/booth-ui/src/camera/CameraCustomerSessionResetPanel.tsx <<'TSX'
import React, {
  useMemo,
  useState,
} from 'react';
...
grep -q "CameraCustomerSessionResetPanel" apps/booth-ui/src/camera/index.ts || cat >> apps/booth-ui/src/camera/index.ts <<'TS'
...
if "CameraCustomerSessionResetPanel" not in text:
    ...
    lines.insert(
        insert_at,
        "import { CameraCustomerSessionResetPanel } from './CameraCustomerSessionResetPanel';",
    )
    text = "\n".join(lines) + "\n"

if "<CameraCustomerSessionResetPanel />" not in text:
    marker = "<CameraToPrintFlowChecklistPanel />"

    if marker in text:
        text = text.replace(
            marker,
            marker + "\n      <CameraCustomerSessionResetPanel />",
            1,
        )
    else:
        fallback = "<CameraPrintQueueSummaryPanel />"

        if fallback not in text:
            raise SystemExit(
                "Could not find CameraToPrintFlowChecklistPanel or CameraPrintQueueSummaryPanel. Put <CameraCustomerSessionResetPanel /> near the bottom of CameraSetupPanel manually."
            )
...
echo ""
echo "Relevant lines:"
grep -R "CameraCustomerSessionResetPanel\\|Customer Session Reset\\|Reset Customer Session" -n apps/booth-ui/src/camera || true
